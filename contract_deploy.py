...
def compile_and_deploy_contract(
    ...
    if txn_receipt:
        ...
        logger.info(f"Saving compiled contract info to {filename}...")
        export_path = f"./sol/contracts/{filename}"
        with open(export_path, "w") as f:
            json.dump({"contract_address": contract_address, "abi": abi}, f)
            logger.info(f"Compiled contract info saved to {contract_name}.json...")
    else:
        logger.error("Contract deploy FAILED...")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test", action='store_true', default=False, help="Testnet deploy")
    ...
    if args.test:
        ...
        logger.info("Deploying to mainnet...")
        provider = Provider(
            wallet_address=config["WALLET_ADDRESS"],
            wallet_private_key=config["WALLET_PRIVATE_KEY"],
            https_url=None,
        ...
        constructor_args = {"_addressProvider": config["AAVE_ARBITRUM_POOL_CONTRACT_ADDRESS_PROVIDER"]}
    else:
        raise Exception("Must specify --test or --main")
